...
export default function Form({setLoading}) {
  ...
  const submitUpdateForm = async () => {
    try {
      ...
      const staff_body = {
        ...
        university_certificate: values.university_certificate,
        special_need_certificate: values.special_need_certificate,
  ...
  React.useEffect(() => {
    console.log('FORM', values, values.has_special_needs)
  }, [values])
      ...
      >
        ...
        <div className="flex items-center mb-6 ">
          {/* <Checkbox
            ...
            value={values.hasSpecialNeeds}
            inputProps={{'aria-label': 'primary checkbox'}}
            onChange={({target: {name, checked}}) =>
              handleChange('hasSpecialNeeds', checked)
            }
          /> */}
          <input
            className="mr-2 p-4 bg-red-500 checkbox"
            type="checkbox"
            checked={values.has_special_needs}
            onChange={({target: {name, checked}}) =>
              handleChange('has_special_needs', checked)
            }
          />
          <div>Ability to work with people with special needs.</div>
        </div>
        ...
        <div className="flex">
          ...
          <DropZone
            ...
            onChange={handleChange}
            value={values.university_certificate}
            padding={3}
            pr={3}
          />
          <DropZone
            square
            name="special_need_certificate"
            label={'Special Needs Certificate'}
            onChange={handleChange}
            value={values.special_need_certificate}
            padding={3}
            pr={3}
          />
        </div>

...
export default function ModelList({query = null, setLoading}) {
  ...
  const rehydrater = async () => {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        let {book} = (await booksApi.get({})) as any
        setDataModel(book)
        resolve(book)
      } catch (e) {
        reject(e)
  ...
  const columns: GridColDef[] = [
    ...
    },
    {
      ...
      renderCell: ({row}) => (
        <div className="tracking-widest">{`${row.user?.phone}`}</div>
      ),
    },
    // {
    //   ...defaultRowConfig,
    ...
    //   headerName: 'Venue',
    //   renderCell: ({row}) => (
    //     <div className="tracking-widest">{`${row.venue?.name?.en}`}</div>
    //   ),
    // },
    {
      ...defaultRowConfig,
      ...
      renderCell: ({row}) => (
        ...
        >
          <p className="text-white">{BOOK_STATUSES[row.status]}</p>
        </div>
      ),
    },
    {
    ...
    },
    {
      ...
      renderCell: ({row}) => (
        <div className="tracking-widest">{`+${row.entity?.business_phone}`}</div>
      ),
    },
    {
      ...defaultRowConfig,
